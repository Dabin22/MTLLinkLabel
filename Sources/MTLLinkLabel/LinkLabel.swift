  ...
  func linkLabelExecuteLink(linkLabel: LinkLabel, text: String, result: NSTextCheckingResult) -> Void {
    ...
    if result.resultType.contains(.link) {
      ...
      if NSPredicate(format: "SELF MATCHES '\(pattern)'").evaluate(with: text) {
        guard let url = URL(string: "mailto:" + text) else { return }
      ...
      guard let url = URL(string: telURLString) else { return }
      UIApplication.shared.open(url, options: [:])
    }
  }
...
public class LinkLabel: UILabel {
   ...
   */
  public func addLink(url: NSURL, range: NSRange, linkAttribute: [NSAttributedString.Key : Any]? = nil, selection: LinkSelection?) -> LinkLabel {
